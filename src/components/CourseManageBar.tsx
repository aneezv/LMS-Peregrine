...
export default function CourseManageBar({ courseId }: { courseId: string }) {
  ...
  async function handleDelete() {
    if (!confirm('Delete this course and all related modules and data? This cannot be undone.')) {
      return
    }
    setDeleting(true)
  ...
  return (
    <div className="flex flex-wrap gap-2 mt-4">
      ...
      <button
        ...
        disabled={deleting}
        className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-red-200 bg-red-50 text-red-800 text-sm font-medium hover:bg-red-100 transition disabled:opacity-50"
      >
